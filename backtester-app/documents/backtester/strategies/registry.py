    ...
    for folder in sorted(strategies_dir.iterdir()):
        ...
        try:
            ...
            if not strat_id or strat_id == "base_strategy":
                logger.error("%s has invalid strategy id: %s", module_name, strat_id)
                continue
            _REGISTRY[strat_id] = strategy_cls
        except Exception as exc:
            logger.error("Failed to load strategy folder %s: %s", folder.name, exc)
...
def get_strategy_by_module(module_name: str) -> Type[BaseStrategy] | None:
    load_all_strategies()
    for sid, cls in _REGISTRY.items():
        if sid.endswith(f"_{module_name}") or cls.__module__.endswith(f".{module_name}"):
            return cls
    return None
