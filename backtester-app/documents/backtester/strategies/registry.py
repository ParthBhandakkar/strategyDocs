"""
Strategy Registry — discovers strategies from subfolders under strategies/.
"""
...
def load_all_strategies() -> None:
    """Import Strategy from each subfolder and register by Strategy.id."""
    ...
    for folder in sorted(strategies_dir.iterdir()):
        ...
        try:
            ...
            if not strat_id or strat_id == "base_strategy":
                logger.error("Strategy in %s has invalid id", module_name)
                continue
...
def get_strategy(strategy_id: str) -> Type[BaseStrategy] | None:
    ...
    for sid, cls in _REGISTRY.items():
        if sid.endswith(f"_{strategy_id}") or cls.__module__.split(".")[-1] == strategy_id:
            return cls
    return None
