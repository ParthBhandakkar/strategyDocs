...
def get_strategy_by_module(module_name: str) -> Type[BaseStrategy] | None:
    load_all_strategies()
    for strat_cls in _REGISTRY.values():
        if module_name in {getattr(strat_cls, "id", ""), getattr(strat_cls, "id", "").split("_", 1)[-1]}:
            return strat_cls
        folder = getattr(strat_cls, "id", "").split("_", 1)[-1]
        if folder == module_name:
            return strat_cls
    for strat_id, strat_cls in _REGISTRY.items():
        if strat_id.endswith(f"_{module_name}") or strat_id == module_name:
            return strat_cls
    return None
