...
def save_result(result, strategy_id: str) -> Path:
...
def main():
    ...
    parser.add_argument("--synthetic", action="store_true", help="Use synthetic OHLCV data")
    ...
    client = SyntheticDataClient(seed=args.seed) if args.synthetic else MT5Client()
    engine = BacktestEngine(config, strat_cls(), client)
    result = engine.run()
    out_path = save_result(result, args.strategy_id)
    client.close()
