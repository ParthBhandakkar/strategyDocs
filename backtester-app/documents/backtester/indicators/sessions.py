...
SESSIONS = {
    ...
    "post_ny_open": {"start": time(9, 30), "end": time(16, 0), "cross": False},
}
...
def get_ny_time(utc_time: datetime) -> datetime:
    """Convert naive UTC timestamp to New York local time."""
    if utc_time.tzinfo is None:
        utc_aware = utc_time.replace(tzinfo=ZoneInfo("UTC"))
    else:
        utc_aware = utc_time.astimezone(ZoneInfo("UTC"))
    return utc_aware.astimezone(NY_TZ)
...
def get_candle_at_time(bars: list[Bar], hour: int, minute: int = 0) -> Optional[Bar]:
    for bar in reversed(bars):
        ...
        if bar_ny.hour == hour and bar_ny.minute == minute:
            return bar
    return None


def get_session_range(bars: list[Bar], session_name: str) -> Optional[SessionRange]:
    session_bars = [bar for bar in bars if is_in_session(bar.time, session_name)]
    if not session_bars:
        return None
    ...
    session_range.high = max(bar.high for bar in session_bars)
    session_range.low = min(bar.low for bar in session_bars)
...
def get_previous_day_high_low(bars: list[Bar], utc_time: datetime) -> tuple[float, float]:
    ...
    day_bars = [bar for bar in bars if get_ny_time(bar.time).date() == target]
    if not day_bars:
        return 0.0, 0.0
    return max(bar.high for bar in day_bars), min(bar.low for bar in day_bars)


def session_bars_for_day(bars: list[Bar], utc_time: datetime, session_name: str) -> list[Bar]:
    """Return bars for a named session on the same NY calendar day as utc_time."""
    day = get_ny_time(utc_time).date()
    return [
        bar
        for bar in bars
        if get_ny_time(bar.time).date() == day and is_in_session(bar.time, session_name)
    ]
