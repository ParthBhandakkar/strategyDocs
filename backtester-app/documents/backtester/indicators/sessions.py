...
def session_open_today(utc_time: datetime, hour: int, minute: int = 0) -> datetime:
    """Return UTC timestamp for a NY session clock time on the current NY date."""
    ny = get_ny_time(utc_time)
    session_start = datetime.combine(ny.date(), time(hour, minute), tzinfo=NY_TZ)
