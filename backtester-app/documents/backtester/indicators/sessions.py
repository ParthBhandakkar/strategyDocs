...
def get_ny_time(utc_time: datetime) -> datetime:
    return _as_utc(utc_time).astimezone(NY_TZ)
...
def session_bars_since_ny_open(bars: list[Bar], current_time: datetime) -> list[Bar]:
    """M1 bars from today's NY 9:30 open through current_time."""
    ...
    if ny_now.time() < time(9, 30):
        return []
    start_utc = session_start.astimezone(timezone.utc).replace(tzinfo=None)
    return [b for b in bars if start_utc <= b.time <= current_time]
