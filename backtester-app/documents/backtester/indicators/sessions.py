...
def get_candle_at_time(bars: list[Bar], hour: int, minute: int = 0) -> Optional[Bar]:
    for bar in reversed(bars):
        ...
        if ny.hour == hour and ny.minute == minute:
            return bar
    return None
...
def get_session_bars_since_open(bars: list[Bar], utc_time: datetime) -> list[Bar]:
    """Return M1 bars from today's NY cash open through current_time."""
    ny_now = get_ny_time(utc_time)
    session_start = datetime.combine(ny_now.date(), time(9, 30), tzinfo=NY_TZ)
    return [
        bar
        for bar in bars
        if get_ny_time(bar.time) >= session_start and bar.time <= utc_time
    ]
