...
SESSIONS = {
    ...
    "london_vp": {"start": time(3, 0), "end": time(7, 0), "cross": False},
}
...
def ny_time_to_utc(ny_dt: datetime) -> datetime:
    if ny_dt.tzinfo is None:
        ny_dt = ny_dt.replace(tzinfo=NY_TZ)
    return ny_dt.astimezone(UTC)


def is_restricted_hour(utc_time: datetime) -> bool:
    ny = get_ny_time(utc_time)
    for rule in RESTRICTED_HOURS.values():
        if rule["start"] <= ny.time() <= rule["end"]:
            return True
    return False
...
def same_ny_date(a: datetime, b: datetime) -> bool:
    return get_ny_time(a).date() == get_ny_time(b).date()
