...
def get_ny_time(utc_time: datetime) -> datetime:
    """Convert UTC timestamp to America/New_York local time."""
...
def is_post_ny_open(utc_time: datetime) -> bool:
    """True when NY cash open (9:30) or later on the same NY calendar day."""
    ny = get_ny_time(utc_time)
...
def session_bars_since_ny_open(bars: list[Bar], current_time: datetime) -> list[Bar]:
    """Bars from 9:30 NY on the current NY session day through current_time."""
    ny_now = get_ny_time(current_time)
    session_start = datetime.combine(ny_now.date(), time(9, 30), tzinfo=NY_TZ)
    if ny_now.time() < time(9, 30):
        return []
    return [b for b in bars if session_start <= get_ny_time(b.time) <= ny_now]
