...
def get_ny_time(utc_time: datetime) -> datetime:
    """Convert UTC (or naive UTC) timestamp to New York local time."""
...
def is_post_ny_open(utc_time: datetime) -> bool:
    """True after 09:30 NY (strategy session filter for cluster A)."""
    ny = get_ny_time(utc_time)
...
def get_previous_day_high_low(bars: list[Bar], utc_time: datetime) -> tuple[float, float]:
    target_date = get_ny_time(utc_time).date() - timedelta(days=1)
    db = [b for b in bars if get_ny_time(b.time).date() == target_date]
    if not db:
        return 0.0, 0.0
