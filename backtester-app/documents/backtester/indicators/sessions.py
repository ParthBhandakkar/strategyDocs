...
def get_ny_time(utc_time: datetime) -> datetime:
    """Convert UTC (or naive-as-UTC) datetime to New York local time."""
...
def ny_session_date(utc_time: datetime):
    """Trading session date in NY (bars before midnight belong to prior session day)."""
    return get_ny_time(utc_time).date()
