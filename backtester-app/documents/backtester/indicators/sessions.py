"""
Session boundaries using America/New_York timezone (DST-aware).
"""
...
def get_ny_time(utc_time: datetime) -> datetime:
    """Return NY-local datetime (DST-aware)."""
    return to_ny(utc_time)
...
def is_after_ny_open(utc_time: datetime) -> bool:
    ny = to_ny(utc_time)
...
def get_session_bars_since_ny_open(bars: list[Bar], current_time: datetime) -> list[Bar]:
    """Bars from today's 9:30 NY open through current_time (inclusive)."""
    ny_now = to_ny(current_time)
    session_start_ny = ny_now.replace(hour=9, minute=30, second=0, microsecond=0)
    if ny_now.time() < time(9, 30):
        return []
    return [b for b in bars if session_start_ny <= to_ny(b.time) <= ny_now]


def get_previous_day_high_low(bars: list[Bar], utc_time: datetime) -> tuple[float, float]:
    target = to_ny(utc_time).date() - timedelta(days=1)
    db = [b for b in bars if to_ny(b.time).date() == target]
    if not db:
        return 0.0, 0.0
