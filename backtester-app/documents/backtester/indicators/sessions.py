...
SESSIONS = {
    ...
    "post_open": {"start": time(9, 30), "end": time(16, 0), "cross": False},
}
...
def get_ny_time(utc_time: datetime) -> datetime:
    ...
    return utc_time.astimezone(NY_TZ).replace(tzinfo=None)
...
def session_bars_for_day(bars: list[Bar], utc_time: datetime, session_name: str = "post_open") -> list[Bar]:
    ny_date = get_ny_time(utc_time).date()
    return [
        bar
        for bar in bars
        if get_ny_time(bar.time).date() == ny_date and is_in_session(bar.time, session_name)
    ]
