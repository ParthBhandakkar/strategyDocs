...
Uses America/New_York timezone (DST-aware).
"""
...
def is_post_ny_open(utc_time: datetime) -> bool:
    """True when NY cash open (9:30) has passed for the session day."""
    ny = get_ny_time(utc_time)
...
def get_session_bars_for_day(bars: list[Bar], utc_time: datetime) -> list[Bar]:
    """Return M1 bars from 9:30 NY open through current time on the same NY day."""
    ny = get_ny_time(utc_time)
    session_start = datetime.combine(ny.date(), time(9, 30), tzinfo=NY_TZ)
    return [
        b
        for b in bars
        if get_ny_time(b.time).date() == ny.date()
        and get_ny_time(b.time) >= session_start
        and b.time <= utc_time
    ]
