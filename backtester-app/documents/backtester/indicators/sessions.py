...
def get_ny_time(utc_time: datetime) -> datetime:
    """Convert UTC (or naive UTC) bar time to America/New_York."""
...
def is_after_ny_open(utc_time: datetime) -> bool:
    """True when NY cash session (9:30+) is active."""
    return is_in_session(utc_time, "new_york") and get_ny_time(utc_time).time() >= time(9, 30)
