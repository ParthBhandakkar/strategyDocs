...
def is_post_ny_open(utc_time: datetime) -> bool:
    return is_in_session(utc_time, "new_york")
...
def session_bars_since_ny_open(bars: list[Bar], current_time: datetime) -> list[Bar]:
    """Return M1 bars from today's NY cash open (09:30) through current_time."""
    ...
    return [
        b
        for b in bars
        if get_ny_time(b.time).date() == session_date
        and get_ny_time(b.time).time() >= time(9, 30)
        and b.time <= current_time
    ]
