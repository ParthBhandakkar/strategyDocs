...
def get_ny_time(utc_time: datetime) -> datetime:
    """Convert naive UTC datetime to New York local time (naive)."""
...
def get_session_bars_since_open(
    bars: list[Bar],
    current_time: datetime,
    session_name: str = "new_york",
) -> list[Bar]:
    """Return session bars from today's session open through current_time."""
    ny_now = get_ny_time(current_time)
    session = SESSIONS[session_name]
    session_start = datetime.combine(ny_now.date(), session["start"])
    result: list[Bar] = []
    for bar in bars:
