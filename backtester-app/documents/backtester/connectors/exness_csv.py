...
Reads OHLCV from folder layout: {data_root}/{SYMBOL}/{tf_folder}/*.csv
"""
...
class ExnessCSVClient:
    ...
    def _find_csv_file(self, symbol: str, tf: TF) -> Optional[Path]:
        folder_name = TF_TO_FOLDER.get(tf)
        if not folder_name:
            return None
    ...
    def get_bars(
        ...
        start_aware = self._ensure_utc(start)
        end_aware = self._ensure_utc(end)
        return [b for b in all_bars if start_aware <= self._ensure_utc(b.time) <= end_aware]
    ...
    def _parse_row(self, row: dict) -> Optional[Bar]:
