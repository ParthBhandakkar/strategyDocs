...
from .data_feed import DataClient, MultiTimeframeDataFeed
...
class BacktestEngine:
    """Event-driven backtesting engine using a local CSV data client."""
    ...
    def __init__(
        ...
        self.broker.set_symbol(config.symbol)
    ...
    def run(self) -> BacktestResult:
        ...
        for event in self.data_feed:
            ...
            if base_bar:
                ...
                for trade, lot_size, commission in closed_trades:
                    self.portfolio.on_trade_closed(trade, lot_size, commission)
        ...
        if last_bar:
            for pos in list(self.broker.open_positions):
                pos.trade.close(
                    ...
                    pos.lot_size,
                    self.broker.pip_value_per_lot,
                )
                commission = self.broker.commission_per_lot * pos.lot_size
                self.portfolio.on_trade_closed(pos.trade, pos.lot_size, commission)
