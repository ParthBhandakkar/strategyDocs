        ...
        for event in self.data_feed:
            ...
            if base_bar:
                ...
                for trade in closed_trades:
                    self.portfolio.on_trade_closed(trade, self.config.symbol)
        ...
        if last_bar:
            for pos in list(self.broker.open_positions):
                pos.trade.close(
                    ...
                    pos.lot_size,
                    self.broker.pip_value_per_lot_usd(self.config.symbol),
                    pos.trade.metadata.get("commission", 0.0),
                )
                self.portfolio.on_trade_closed(pos.trade, self.config.symbol)
