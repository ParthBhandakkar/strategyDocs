"""
Event-driven backtest engine using injectable local CSV data client.
"""
...
class BacktestEngine:
    """Runs a strategy over historical bars from a local data client."""
    ...
    def __init__(
        ...
        extra_symbols = getattr(strategy, "extra_symbols", [])
    ...
    def run(self) -> BacktestResult:
        ...
        if last_bar and self.broker.open_positions:
            open_count = len(self.broker.open_positions)
            self.broker.force_close_all(last_bar)
            for trade in self.broker.closed_trades[-open_count:]:
                self.portfolio.on_trade_closed(trade)
