...
class MultiTimeframeDataFeed:
    ...
    Emits MarketEvents when base bars close; HTF bars appear only after close.
    """
    ...
    def __iter__(self):
        ...
        for base_bar in base_bars:
            # Event time = bar close (signals evaluated at close)
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    while idx < len(tf_bars):
                        ...
                        bar_close = candidate.time + tf_duration
