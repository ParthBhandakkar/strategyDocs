...
class MultiTimeframeDataFeed:
    ...
    def __init__(
        ...
        self._last_emitted: dict[str, dict[TF, datetime | None]] = defaultdict(
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    while idx < len(tf_bars):
                        ...
                        bar_close_time = candidate.time + bar_duration
                    ...
                    if history:
                        latest = history[-1]
                        last_emitted = self._last_emitted[sym][tf]
                        if last_emitted != latest.time:
                            self._last_emitted[sym][tf] = latest.time
                            if sym == self.symbol:
                                new_bars[tf] = latest
