...
HTF bars enter history only after bar close (bar.time + tf_duration <= current_time).
"""
...
class MultiTimeframeDataFeed:
    """Advances on base timeframe; exposes HTF bars only after close."""
        ...
        for base_bar in base_bars:
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    bar_close_deadline = self._current_time

                    while idx < len(tf_bars):
                        candidate = tf_bars[idx]
                        close_time = candidate.time + timedelta(minutes=tf_minutes)
                        if close_time > bar_close_deadline:
                            break
    ...
    def get_history(
        self, symbol: str | None = None, timeframe: TF | None = None, lookback: int = 100
    ) -> list[Bar]:
