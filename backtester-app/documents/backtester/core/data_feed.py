...
class MultiTimeframeDataFeed:
    ...
    when bars complete on subscribed timeframes.
    """
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    while idx < len(tf_bars):
                        ...
                        if not _bar_is_closed(candidate, tf, self._current_time):
                            break
...
def _bar_is_closed(bar: Bar, timeframe: TF, current_time: datetime) -> bool:
    """Bar is visible only after its period closes."""
    bar_time = bar.time
    ...
    close_time = bar_time + timedelta(minutes=tf_to_minutes(timeframe))
    return close_time <= cur
