...
class MultiTimeframeDataFeed:
    """Advances time on the smallest TF and emits events when bars close."""
    ...
    def __init__(
        ...
        self._last_emitted: dict[str, dict[TF, Optional[datetime]]] = defaultdict(
    ...
    def _bar_close_time(self, bar: Bar, tf: TF) -> datetime:
        close = bar.time + timedelta(minutes=tf_to_minutes(tf))
        if close.tzinfo is None:
            close = close.replace(tzinfo=timezone.utc)
        return close

    def _advance_closed_bars(self, sym: str, tf: TF, current_time: datetime) -> Optional[Bar]:
        tf_bars = self._all_bars.get(sym, {}).get(tf, [])
        idx = self._next_index[sym][tf]
        latest_closed: Optional[Bar] = None

        while idx < len(tf_bars):
            candidate = tf_bars[idx]
            if self._bar_close_time(candidate, tf) <= current_time:
                self._history[sym][tf].append(candidate)
                latest_closed = candidate
                idx += 1
            else:
                break
    ...
    def __iter__(self):
        ...
        for base_bar in base_bars:
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    closed = self._advance_closed_bars(sym, tf, self._current_time)
                    if closed and self._last_emitted[sym][tf] != closed.time:
                        self._last_emitted[sym][tf] = closed.time
                        if sym == self.symbol:
                            new_bars[tf] = closed
                        multi_bars[sym][tf] = closed
