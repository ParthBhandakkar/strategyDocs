...
class DataClient(Protocol):
    def get_bars(
        self, symbol: str, timeframe: TF, start: datetime, end: datetime
    ) -> list[Bar]: ...


class MultiTimeframeDataFeed:
    """
    Advances time by the smallest subscribed timeframe.
    Higher-TF bars are exposed only after their period closes.
    """
    ...
    def __init__(
        ...
        self._last_emitted: dict[str, dict[TF, Bar | None]] = defaultdict(dict)

        self._loaded = False
        self._current_time: Optional[datetime] = None

    def _bar_closed(self, bar: Bar, tf: TF, current_time: datetime) -> bool:
        duration = timedelta(minutes=tf_to_minutes(tf))
        return bar.time + duration <= current_time
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    while idx < len(tf_bars):
                        ...
                        if not self._bar_closed(candidate, tf, self._current_time):
                            break
                    ...
                    if history:
                        latest = history[-1]
                        prev = self._last_emitted[sym].get(tf)
                        if prev is None or latest.time != prev.time:
                            if sym == self.symbol:
                                new_bars[tf] = latest
                            multi_bars[sym][tf] = latest
                            self._last_emitted[sym][tf] = latest
    ...
    def get_current_bar(
        self, symbol: str | None = None, timeframe: TF | None = None
    ) -> Optional[Bar]:
