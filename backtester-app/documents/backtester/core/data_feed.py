...
class MultiTimeframeDataFeed:
    """Advances by base timeframe and emits MarketEvents on bar completion."""
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    while idx < len(tf_bars):
                        ...
                        bar_close = candidate.time + timedelta(minutes=tf_minutes)
                        if bar_close <= self._current_time:
                            self._history[sym][tf].append(candidate)
                            prev = self._last_emitted[sym][tf]
                            if prev is None or candidate.time > prev.time:
                                if sym == self.symbol:
