...
class MultiTimeframeDataFeed:
    ...
    when bars complete. HTF bars enter history only after their period closes.
    """
    ...
    def load(self):
        ...
        if base_bars:
            self._current_time = base_bars[0].time + timedelta(minutes=tf_to_minutes(self.base_tf))
        ...
        for base_bar in base_bars:
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    tf_duration = timedelta(minutes=tf_to_minutes(tf))

                    while idx < len(tf_bars) and tf_bars[idx].time <= self._current_time:
                        candidate = tf_bars[idx]
                        if candidate.time + tf_duration <= self._current_time:
