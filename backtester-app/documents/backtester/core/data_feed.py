...
class MultiTimeframeDataFeed:
    """Advances time bar-by-bar and emits MarketEvents on completed bars."""
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    while idx < len(tf_bars):
                        ...
                        if self._bar_close_time(candidate, tf) > self._current_time:
                            break
                        self._history[sym][tf].append(candidate)
                        idx += 1

                    self._indices[sym][tf] = idx
                    if idx > prev_idx and self._history[sym][tf]:
                        latest = self._history[sym][tf][-1]
                        if sym == self.symbol:
                            new_bars[tf] = latest
                        multi_bars[sym][tf] = latest
                    elif self._history[sym][tf]:
                        multi_bars[sym][tf] = self._history[sym][tf][-1]
    ...
    def _bar_close_time(bar: Bar, tf: TF) -> datetime:
        ...
        return bar_time + timedelta(minutes=tf_to_minutes(tf))
