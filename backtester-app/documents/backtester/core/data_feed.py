...
class MultiTimeframeDataFeed:
    ...
    def __init__(
        ...
        self._pending_htf: dict[str, dict[TF, list[Bar]]] = defaultdict(lambda: defaultdict(list))
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    while idx < len(tf_bars) and tf_bars[idx].time <= self._current_time:
                        bar = tf_bars[idx]
                        bar_close_time = bar.time + timedelta(minutes=tf_minutes)
                        if tf == self.base_tf or bar_close_time <= self._current_time:
                            self._history[sym][tf].append(bar)
                        else:
                            self._pending_htf[sym][tf].append(bar)
                        idx += 1

                    self._indices[sym][tf] = idx

                    self._release_closed_htf_bars(sym, tf)
    ...
    def _release_closed_htf_bars(self, sym: str, tf: TF):
        """Expose HTF bars only after their period has closed."""
        if tf == self.base_tf:
            return
        tf_minutes = tf_to_minutes(tf)
        pending = self._pending_htf[sym][tf]
        still_pending: list[Bar] = []
        for bar in pending:
            bar_close_time = bar.time + timedelta(minutes=tf_minutes)
            if bar_close_time <= self._current_time:
                self._history[sym][tf].append(bar)
            else:
                still_pending.append(bar)
        self._pending_htf[sym][tf] = still_pending
