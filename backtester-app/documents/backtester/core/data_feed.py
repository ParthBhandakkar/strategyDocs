...
class MultiTimeframeDataFeed:
    """Advances on the smallest timeframe and emits closed bars only."""
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    while idx < len(tf_bars):
                        ...
                        if candidate.time + close_offset > self._current_time:
                            break
                        self._history[sym][tf].append(candidate)
                        previous = self._last_emitted[sym][tf]
                        if previous is None or candidate.time > previous.time:
                            if sym == self.symbol:
