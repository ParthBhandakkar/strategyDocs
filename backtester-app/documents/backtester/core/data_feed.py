...
class MultiTimeframeDataFeed:
    """Advances on the smallest TF and emits events when bars close."""
    ...
    def _bar_is_closed(self, bar: Bar, tf: TF, current_time: datetime) -> bool:
        ...
        bc = bar_close if bar_close.tzinfo else bar_close.replace(tzinfo=timezone.utc)
        return bc <= ct
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    while idx < len(tf_bars):
                        ...
                        if not self._bar_is_closed(candidate, tf, self._current_time):
                            break
