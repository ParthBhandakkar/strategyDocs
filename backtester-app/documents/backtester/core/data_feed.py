"""
Multi-Timeframe Data Feed with HTF anti-lookahead.
"""
...
class MultiTimeframeDataFeed:
    ...
    HTF bars enter history only after bar close (bar.time + duration <= current_time).
    """
    ...
    def _advance_history(self, sym: str, tf: TF, current_time: datetime) -> Optional[Bar]:
        tf_bars = self._all_bars.get(sym, {}).get(tf, [])
        history = self._history[sym][tf]
        start_idx = len(history)
        newly_closed: Optional[Bar] = None

        for bar in tf_bars[start_idx:]:
            if bar.time > current_time:
                break
            if self._bar_is_closed(bar, tf, current_time):
                history.append(bar)
                newly_closed = bar
            else:
                break
    ...
    def __iter__(self):
        ...
        for base_bar in base_bars:
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    latest = self._advance_history(sym, tf, self._current_time)
                    if latest is None:
                        history = self._history[sym][tf]
