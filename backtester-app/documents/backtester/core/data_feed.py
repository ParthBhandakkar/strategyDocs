...
from backtester.core.timeframes import TF, sort_timeframes, tf_to_minutes
        ...
        for base_bar in base_bars:
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    close_offset = timedelta(minutes=tf_minutes)

                    while idx < len(tf_bars):
                        candidate = tf_bars[idx]
                        bar_close_time = candidate.time + close_offset
                        if bar_close_time > self._current_time:
                            break

                        self._history[sym][tf].append(candidate)
                        self._last_emitted[sym][tf] = candidate.time
                        idx += 1
