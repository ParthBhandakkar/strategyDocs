...
class MultiTimeframeDataFeed:
    ...
    Higher-TF bars are exposed in history only after period close.
    """
    ...
    def _bar_is_closed(self, bar: Bar, tf: TF, current_time: datetime) -> bool:
        ...
        bar_end = bar.time + duration
        ct = current_time if current_time.tzinfo else current_time.replace(tzinfo=timezone.utc)
        be = bar_end if bar_end.tzinfo else bar_end.replace(tzinfo=timezone.utc)
        return be <= ct
            ...
            for sym in [self.symbol] + self.extra_symbols:
                for tf in self.timeframes:
                    ...
                    while idx < len(tf_bars):
                        ...
                        if candidate.time > self._current_time:
                            break
