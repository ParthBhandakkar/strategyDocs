...
class SimulatedBroker:
    ...
    def pip_value_usd_per_lot(self, symbol: str) -> float:
        ...
        if "ETH" in sym:
            return 0.1
        if "JPY" in sym and sym.startswith("USD"):
            return 6.5
        if "JPY" in sym:
            return 7.0
        return 10.0
    ...
    def execute_signal(
        ...
        pip_value_per_lot = self.pip_value_usd_per_lot(signal.symbol)
        ...
        trade = Trade(
            ...
            metadata={**signal.metadata.copy(), "lot_size": lot_size, "commission_usd": commission},
        )
