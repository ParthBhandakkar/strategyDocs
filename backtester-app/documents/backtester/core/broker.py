...
Symbol-aware pip value for comparable risk across instruments.
"""
...
def pip_value_per_lot(symbol: str, price: float) -> float:
    ...
    if sym.endswith("USD"):
        return 10.0
    if sym.startswith("USD") and "JPY" in sym:
        return (CONTRACT_SIZE * pip) / max(price, 1e-9)
    if "JPY" in sym:
        return (CONTRACT_SIZE * pip) / max(price, 1e-9)
    return 10.0


def price_delta_to_usd(symbol: str, delta: float, lot_size: float, price: float) -> float:
    pip = pip_size_for_symbol(symbol)
    if pip <= 0:
        return 0.0
    pips = delta / pip
    return pips * pip_value_per_lot(symbol, price) * lot_size
...
class SimulatedBroker:
    ...
    def execute_signal(
        ...
        pvpl = pip_value_per_lot(signal.symbol, signal.entry_price)
        lot_size = risk_amount / (pip_distance * pvpl)
