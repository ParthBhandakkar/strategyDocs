...
class SimulatedBroker:
    ...
    def set_symbol(self, symbol: str):
        """Configure pip sizing for a symbol."""
        self.symbol = symbol.upper()
        sym = self.symbol
        if "JPY" in sym:
            self.pip_value = 0.01
            self.pip_value_per_lot = 6.5 if sym.endswith("JPY") and not sym.startswith("USD") else 10.0
    ...
    def execute_signal(
        ...
        trade = Trade(
            ...
            metadata={**signal.metadata, "lot_size": lot_size, "entry_commission": commission},
        )
        self._next_trade_id += 1
    ...
    def update_positions(self, bar: Bar, step_tracker: StepTracker | None = None) -> list[tuple[Trade, float, float]]:
