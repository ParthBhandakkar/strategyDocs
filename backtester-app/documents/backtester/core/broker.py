...
Symbol-aware pip sizing and USD risk model.
"""
...
def usd_per_pip_per_lot(symbol: str) -> float:
    sym = symbol.upper()
    if "XAU" in sym:
        return 1.0
    if "XAG" in sym:
...
class SimulatedBroker:
    ...
    def execute_signal(
        ...
        pip_value_per_lot = usd_per_pip_per_lot(signal.symbol)
    ...
    def compute_trade_pnl_usd(self, trade: Trade) -> float:
        if trade.exit_price is None:
            return 0.0
        price_move = trade.pnl
        pip_move = price_move / self.pip_value if self.pip_value else 0.0
        lot_size = float(trade.metadata.get("lot_size", 0.01))
        gross = pip_move * usd_per_pip_per_lot(trade.symbol) * lot_size
