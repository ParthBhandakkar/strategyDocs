...
class SimulatedBroker:
    ...
    def execute_signal(
        ...
        pip_value_lot = pip_value_per_lot_usd(signal.symbol, signal.entry_price)
        lot_size = risk_amount / (pip_distance * pip_value_lot)
        ...
        metadata["commission_usd"] = round(commission, 2)
    ...
    def compute_trade_pnl_usd(self, trade: Trade, exit_price: float) -> float:
        lot_size = float(trade.metadata.get("lot_size", 0.01))
        commission = float(trade.metadata.get("commission_usd", self.commission_per_lot * lot_size))
        pip_value_lot = pip_value_per_lot_usd(trade.symbol, trade.entry_price)
        ...
        pips = price_move / self.pip_value
        gross = pips * pip_value_lot * lot_size
    ...
    def update_positions(self, bar: Bar, step_tracker: StepTracker | None = None) -> list[Trade]:
        ...
        for pos in self.open_positions:
            ...
            if pos.is_sl_hit(bar):
                ...
                trade.metadata["pnl_usd"] = self.compute_trade_pnl_usd(trade, exit_price)
                closed_trade = trade
                if step_tracker:
    ...
    def force_close_all(self, bar: Bar) -> list[Trade]:
        ...
        for pos in list(self.open_positions):
            trade = pos.trade
            trade.close(bar.time, bar.close, self.pip_value)
            trade.metadata["pnl_usd"] = self.compute_trade_pnl_usd(trade, bar.close)
