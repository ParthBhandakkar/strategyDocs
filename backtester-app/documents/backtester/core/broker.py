...
def pip_value_per_lot_usd(symbol: str) -> float:
    """Approximate USD value of one pip per standard lot."""
    sym = symbol.upper()
    if "XAU" in sym:
        return 10.0
    if "XAG" in sym:
...
class SimulatedBroker:
    ...
    def __init__(
        ...
        self._pip_value_per_lot = pip_value_per_lot_usd(symbol)
    ...
    def execute_signal(
        ...
        lot_size = risk_amount / (pip_distance * self._pip_value_per_lot)
