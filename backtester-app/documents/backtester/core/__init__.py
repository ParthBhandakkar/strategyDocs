...
class Trade:
    ...
    def close(
        ...
        self.metadata["pnl_usd"] = round(self.pnl_pips * pip_value_per_lot * lot_size, 2)
...
class BacktestResult:
    ...
    def compute_stats(self):
        ...
        winners = [t for t in self.trades if float(t.metadata.get("pnl_usd_net", t.metadata.get("pnl_usd", 0.0))) > 0]
        losers = [t for t in self.trades if float(t.metadata.get("pnl_usd_net", t.metadata.get("pnl_usd", 0.0))) <= 0]
        ...
        gross_profit = sum(float(t.metadata.get("pnl_usd_net", t.metadata.get("pnl_usd", 0.0))) for t in winners) if winners else 0
        gross_loss = abs(sum(float(t.metadata.get("pnl_usd_net", t.metadata.get("pnl_usd", 0.0))) for t in losers)) if losers else 0
        self.profit_factor = round(gross_profit / gross_loss, 2) if gross_loss > 0 else float('inf')

        self.total_pnl = round(
            sum(float(t.metadata.get("pnl_usd_net", t.metadata.get("pnl_usd", 0.0))) for t in self.trades),
            2,
        )
        self.avg_rr = round(
