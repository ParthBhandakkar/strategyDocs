...
class Trade:
    ...
    def close(
        ...
        """Close the trade and calculate USD PnL."""
...
class BacktestResult:
    ...
    def compute_stats(self):
        ...
        def trade_pnl_for_filter(trade: Trade) -> float:
            return float(trade.metadata.get("pnl_usd", trade.pnl))

        winners = [t for t in self.trades if trade_pnl_for_filter(t) > 0]
        losers = [t for t in self.trades if trade_pnl_for_filter(t) <= 0]
        ...
        def trade_pnl(trade: Trade) -> float:
            return float(trade.metadata.get("pnl_usd", trade.pnl))

        gross_profit = sum(trade_pnl(t) for t in winners) if winners else 0
        gross_loss = abs(sum(trade_pnl(t) for t in losers)) if losers else 0
        self.profit_factor = round(gross_profit / gross_loss, 2) if gross_loss > 0 else float("inf")

        self.total_pnl = round(sum(trade_pnl(t) for t in self.trades), 2)
        self.avg_rr = round(
