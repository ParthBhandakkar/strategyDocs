...
class Portfolio:
    ...
    def on_trade_closed(self, trade: Trade, lot_size: float, commission: float):
        pnl_usd = float(trade.metadata.get("pnl_usd", 0.0)) - commission
        trade.metadata["pnl_usd_net"] = round(pnl_usd, 2)
