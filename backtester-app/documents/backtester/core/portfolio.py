...
class Portfolio:
    ...
    def on_trade_closed(self, trade: Trade, lot_size: float = 0.01, pip_value: float = 0.0001):
        """Update portfolio when a trade is closed using USD PnL."""
        symbol = trade.symbol
        pip_per_lot = pip_value_per_lot_usd(symbol)
        pnl_pips = trade.pnl / pip_value if pip_value > 0 else 0.0
        commission = self.config.commission_per_lot * lot_size
        pnl_usd = (pnl_pips * pip_per_lot * lot_size) - commission
